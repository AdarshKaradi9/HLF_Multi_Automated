...
chaincodeQuery() {
  ...
  do
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n logchain -c '{"Args":["queryLog","user1"]}' >&log.txt
    res=$?
    set +x
...
#'{"function":"addLogs","Args":["user1","1","2","3","80","1"]}'
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    set -x
    peer chaincode invoke -o orderer0.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n logchain $PEER_CONN_PARMS -c '{"function":"addLogs","Args":["user1","1","2","90","1"]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
