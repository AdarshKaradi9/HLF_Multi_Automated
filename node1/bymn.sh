...
function replacePrivateKey() {
  ...
  cp $TEMPLATE_DIR/docker-compose-ca1-template.yaml $CA_SCRIPTS_DIR/docker-compose-ca1.yaml
  cp $TEMPLATE_DIR/docker-compose-ca2-template.yaml $CA_SCRIPTS_DIR/docker-compose-ca2.yaml
  # The next steps will replace the template's contents with the
  # actual values of the private key file names for the two CAs.
  cd crypto-config/peerOrganizations/org1.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" ca_scripts/docker-compose-ca1.yaml
   cd crypto-config/peerOrganizations/org2.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" ca_scripts/docker-compose-ca2.yaml
